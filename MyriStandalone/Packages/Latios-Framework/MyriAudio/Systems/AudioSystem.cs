...
namespace Latios.Myri.Systems
{
    ...
    public class AudioSystem : SystemBase
    {
        ...
        AudioDestroyCommandBufferSystem mAudioDestroyCommandBufferSystem;
        ...
        protected override void OnCreate()
        {
            ...
            //Create queries
            m_aliveListenersQuery                = GetEntityQuery(ComponentType.ReadOnly<AudioListener>()); //Fluent.WithAll<AudioListener>(true).Build();
            m_deadListenersQuery                 = GetEntityQuery(ComponentType.Exclude<AudioListener>(), ComponentType.ReadWrite<ListenerGraphState>()); //Fluent.Without<AudioListener>().WithAll<ListenerGraphState>().Build();
            m_oneshotsToDestroyWhenFinishedQuery = GetEntityQuery(ComponentType.ReadWrite<AudioSourceOneShot>(), ComponentType.ReadOnly<AudioSourceDestroyOneShotWhenFinished>()); //Fluent.WithAll<AudioSourceOneShot>().WithAll<AudioSourceDestroyOneShotWhenFinished>(true).Build();
            m_oneshotsQuery                      = GetEntityQuery(ComponentType.ReadWrite<AudioSourceOneShot>());//Fluent.WithAll<AudioSourceOneShot>().Build();
            m_loopedQuery                        = GetEntityQuery(ComponentType.ReadWrite<AudioSourceLooped>());//Fluent.WithAll<AudioSourceLooped>().Build();
            ...
            mAudioDestroyCommandBufferSystem = World.GetOrCreateSystem<AudioDestroyCommandBufferSystem>();
        }

        protected override void OnUpdate()
        {
            var worldBlackboardEntity = GetSingletonEntity<AudioSettings>();
            var worldBlackboardData = GetSingleton<AudioSettings>();
            ...
            //Containers
            var destroyCommandBuffer    = mAudioDestroyCommandBufferSystem.CreateCommandBuffer().AsParallelWriter();
            var entityCommandBuffer     = m_endSimulationEcbSystem.CreateCommandBuffer();
            ...
            m_endSimulationEcbSystem.AddJobHandleForProducer(updateListenersGraphJH);
            ...
            m_endSimulationEcbSystem.AddJobHandleForProducer(destroyOneshotsJH);
